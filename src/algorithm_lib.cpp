...
//    M(2,2) = 7.510809364927562E-2;
    ...
    M(0,1) = 0;//t10;
    M(0,2) = 0;//t8-t2*t4*1.497491323180332E-3-t3*t4*8.442002607441242E-3-t2*t7*8.442002607441242E-3;
    M(1,0) = 0;//t10;
    M(1,1) = t2*(-3.823901416739271E-2)-t3*2.436916887631973E-1+3.25213293522124E-1;
    M(1,2) = t14;
    M(2,0) = 0;//t8-t11-t12-t13;
    M(2,1) = t14;
    M(2,2) = 1.167286826846518E-1;
}

void ComputedTorqueController::do_computed_torque_control(Eigen::Vector3d &v_q, Eigen::Vector3d &v_dq, Eigen::Vector3d &v_dq_filtered,
                                                          ...
                                                          Eigen::Vector3d &v_ddqd, Eigen::Vector3d &v_tau)
{
    ...
    coriolis_term_torque(param, v_q, v_dq_filtered, v_c);
...
////    computed_torque(param, q, dq, a, tau);
    ...
    if(cnt%500 == 0)
    {
        ...
        cout << "gravity torque: " << v_g.transpose() << endl;
        cout << "coriolis torque: " << v_c.transpose() << endl << endl;
    }
